...
router.patch(
    ...
    validateRequest,
    clinicalNoteController.completeExistingNote
);

// Update the existing complete route description for clarity
router.post('/complete',
    authenticateToken,
    authorizeRole(['admin', 'therapist', 'staff']),
    [
        body('clinicalNoteData').isObject().withMessage('Clinical note data is required'),
        body('clinicalNoteData.assessment').notEmpty().withMessage('Assessment is required'),
        body('anatomicalMarkings').isObject().withMessage('Anatomical markings data is required'),
    ],
    validateRequest,
