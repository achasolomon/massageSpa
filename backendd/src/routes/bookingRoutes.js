...
router.post(
    ...
    [
       ...
       body("therapistId").optional().custom((value) => {
            // Allow null, undefined, empty string, or "any" for no therapist assignment
            if (value === "any" || value === null || value === undefined || value === "") {
                return true;
            }
            // For actual therapist IDs, validate they are UUIDs
            if (typeof value === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) {
                return true;
            }
            throw new Error('Therapist ID must be a valid UUID or null');
        }),
...
router.get(
  ...
  validateRequest,
  bookingController.getReadyForNotes
);
...
router.put(
    ...
    [
        param("id").isUUID().withMessage("Booking ID must be a UUID"),
        body("therapistId").optional({ nullable: true, checkFalsy: true }).isUUID().withMessage("Therapist ID must be UUID"),
